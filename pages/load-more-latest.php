...
if(!empty($_POST["id"])){
    ...
    //--------- from post db ----
      //count bre articles
    $pid = $_POST['id'];
    $sql = "SELECT COUNT(p_id) as nbre FROM post WHERE p_id < $pid";
    $r = $conn->query($sql);
    $row = $r->fetch_assoc();
    $nbre = $row["nbre"];
     //slect articles
    $sql = "SELECT p_title, p_image, p_content, p_date, p_time, p_views, p_cat, p_id FROM post WHERE p_id < $pid ORDER BY p_id DESC LIMIT 6";
    $result = $conn->query($sql);
    $i = 1;
    if ($result->num_rows > 0) {
